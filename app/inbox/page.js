...
// Statuses where pinning is automatically cleared
...
export default function DMInbox() {
  ...
  async function updateStatus(leadId, status) {
    ...
    // Auto-unpin when booked or disqualified
  ...
  // ── Derived data ─────────────────────────────────────────────────────────────
  // Apply derived status + write ghosted back to Supabase
  ...
  return (
    <div className="flex h-screen font-sans" style={{ background: '#0a0a0a' }}>
              ...
              <div key={lead.id} onClick={() => selectLead(lead)}
                ...
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    {/* Top row: star + name + platform + time */}
                    ...
                    {/* Bottom row: last message preview + status + unread dot */}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Brand header (nav buttons removed per request) */}
        <div className="flex items-center justify-center gap-3 py-5 border-b flex-shrink-0"
          style={{ background: '#111', borderColor: '#222' }}>
          <DumbbellIcon size={28} />
          <div className="text-center">
            <h1 className="font-bold tracking-widest text-lg" style={{ color: '#B8935A' }}>LARGE DUMBBELLS</h1>
            <p className="text-xs" style={{ color: '#444' }}>DM Inbox</p>
          </div>
          <DumbbellIcon size={28} />
        </div>
