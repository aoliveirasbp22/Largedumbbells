...
// Mobile: <768px (phones, portrait small tablets)
// Tablet: 768-1023px (tablets, narrow laptops)
// Desktop: >=1024px
...
// useIsMobile() — returns true on viewports narrower than `maxWidth` (default 768).
// Updates on resize. Safe for SSR (returns false until mounted).
export function useIsMobile(maxWidth = BREAKPOINTS.mobile) {
...
// ─── Phone formatting ─────────────────────────────────────────────────
// Uses libphonenumber-js to format any country's number into its
// canonical international form, e.g. "+1 302 668 5417", "+44 20 7946 0958".
...
export function formatPhone(raw, defaultCountry = 'US') {
  ...
  try {
    ...
    if (parsed && parsed.isValid()) {
      return parsed.formatInternational() // e.g. "+1 302 668 5417"
    }
  } catch {}
  return raw
}

// Returns the E.164 version (e.g. "+13026685417") suitable for tel: links.
// Returns raw if it can't parse, since tel: still works on most clients.
export function phoneHref(raw, defaultCountry = 'US') {
  if (!raw) return ''
  try {
    const parsed = parsePhoneNumberFromString(String(raw), defaultCountry)
...
// Desktop: left slot | centered brand+label | right slot
// Mobile: tight row with left slot, smaller brand, right slot; label moves under brand inline
    ...
    <div style={{
      ...
      <div style={{
        ...
        <img
          ...
          alt="Large Dumbbells"
          style={{
            width: isMobile ? mobileLogo : logoSize,
            height: isMobile ? mobileLogo : logoSize,
            objectFit: 'contain',
          }}
        />
        {pageLabel && (
